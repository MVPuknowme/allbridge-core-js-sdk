...
// Allbridge: per-package type-origin overrides set via setPackageAddress(). When
// present, getTypeOrigin returns this address for every type in the package
// (Allbridge packages define all their types at the original/v1 address).
let typeOriginOverrides: Record<string, string> = {};
 ...
 * @param overrides - Optional map of package names to publishedAt addresses to override
 */
export function setActiveEnvWithConfig(config: EnvConfig, overrides?: Record<string, string>): void {
  activeEnv = config;
...
/**
 * Override a single package's addresses at runtime, without resetting the active
 * environment or other overrides.
 *
 * Backs the per-package `setAddress()` helpers (Allbridge customization): callers
 * inject contract addresses obtained at runtime (e.g. from an API). `publishedAt`
 * is used for function-call targets; `originalId` (defaulting to `publishedAt`) is
 * used as the type origin for every type in the package, making the generated code
 * network-agnostic without a baked per-network env.
 */
export function setPackageAddress(pkgName: string, publishedAt: string, originalId?: string): void {
  publishedAtOverrides[pkgName] = publishedAt;
  typeOriginOverrides[pkgName] = originalId ?? publishedAt;
}
...
export function getTypeOrigin(pkgName: string, moduleTypePath: string, env?: EnvConfig): string {
  // Allbridge: a runtime override (set via setPackageAddress) wins over the active
  // env, but never over an explicitly supplied env (callers own that scope).
  if (!env && typeOriginOverrides[pkgName]) {
    return typeOriginOverrides[pkgName];
  }
  const source = resolveEnv(env);
 ...
 */
export function getTypeOriginAddressesFor(pkgName: string, moduleTypePaths: string[], env?: EnvConfig): string[] {
  const source = resolveEnv(env);
  const config = source.packages[pkgName] || source.dependencies[pkgName];
  if (!config) {
    throw new Error(`Package '${pkgName}' not found in ${envScopeLabel(env)}`);
