...
export class Element<T extends PhantomTypeArgument> implements StructClass {
  ...
  static reified<T extends PhantomReified<PhantomTypeArgument>>(T: T): ElementReified<ToPhantomTypeArgument<T>> {
    ...
    return {
      ...
      fromBcs: (data: Uint8Array) => Element.fromFields(T, reifiedBcs.parse(data)),
      bcs: reifiedBcs,
      fromJSONField: (field: any) => Element.fromJSONField(T, field),
      fromJSON: (json: Record<string, any>) => Element.fromJSON(T, json),
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => Element.fromCoreObject(T, obj),
  ...
  static fromJSON<T extends PhantomReified<PhantomTypeArgument>>(
    ...
    if (json.$typeName !== Element.$typeName) {
      throw new Error(`not a Element json object: expected '${Element.$typeName}' but got '${json.$typeName}'`);
    }
    assertReifiedTypeArgsMatch(composeSuiType(Element.$typeName, ...[extractType(typeArg)]), json.$typeArgs, [typeArg]);
  ...
  static fromCoreObject<T extends PhantomReified<PhantomTypeArgument>>(
    ...
    if (!isElement(obj.type)) {
      throw new Error(`object at ${obj.objectId} is not a Element object`);
    }
  ...
  static async fetch<T extends PhantomReified<PhantomTypeArgument>>(
    ...
    return Element.fromBcs(typeArg, object.content);
  }
}
