...
export class VerifiedID implements StructClass {
  ...
  static readonly $typeName: `0x2::zklogin_verified_id::VerifiedID` = `0x2::zklogin_verified_id::VerifiedID` as const;
  ...
  private constructor(typeArgs: [], fields: VerifiedIDFields) {
    this.$fullTypeName = composeSuiType(VerifiedID.$typeName, ...typeArgs) as `0x2::zklogin_verified_id::VerifiedID`;
  ...
  static reified(): VerifiedIDReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(VerifiedID.$typeName, ...[]) as `0x2::zklogin_verified_id::VerifiedID`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => VerifiedID.fromCoreObject(obj),
  ...
  static fromJSON(json: Record<string, any>): VerifiedID {
    if (json.$typeName !== VerifiedID.$typeName) {
      throw new Error(`not a VerifiedID json object: expected '${VerifiedID.$typeName}' but got '${json.$typeName}'`);
    }
