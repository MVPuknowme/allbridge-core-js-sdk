...
export class Referent<T extends TypeArgument> implements StructClass {
  ...
  private constructor(typeArgs: [ToTypeStr<T>], fields: ReferentFields<T>) {
    this.$fullTypeName = composeSuiType(Referent.$typeName, ...typeArgs) as `0x2::borrow::Referent<${ToTypeStr<T>}>`;
  ...
  static reified<T extends Reified<TypeArgument, any>>(T: T): ReferentReified<ToTypeArgument<T>> {
    ...
    return {
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => Referent.fromCoreObject(T, obj),
  ...
  static fromJSON<T extends Reified<TypeArgument, any>>(
    ...
    if (json.$typeName !== Referent.$typeName) {
      throw new Error(`not a Referent json object: expected '${Referent.$typeName}' but got '${json.$typeName}'`);
    }
    assertReifiedTypeArgsMatch(composeSuiType(Referent.$typeName, ...[extractType(typeArg)]), json.$typeArgs, [
      typeArg,
    ]);
...
export class Borrow implements StructClass {
  ...
  static reified(): BorrowReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(Borrow.$typeName, ...[]) as `0x2::borrow::Borrow`;
      },
      typeArgs: [] as [],
