...
export class ID implements StructClass {
  ...
  static reified(): IDReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(ID.$typeName, ...[]) as `0x2::object::ID`;
      },
      typeArgs: [] as [],
...
export class UID implements StructClass {
  ...
  static reified(): UIDReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(UID.$typeName, ...[]) as `0x2::object::UID`;
      },
      typeArgs: [] as [],
  ...
  static fromJSON(json: Record<string, any>): UID {
    if (json.$typeName !== UID.$typeName) {
      throw new Error(`not a UID json object: expected '${UID.$typeName}' but got '${json.$typeName}'`);
    }
