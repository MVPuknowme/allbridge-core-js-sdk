...
export class Bag implements StructClass {
  ...
  static reified(): BagReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(Bag.$typeName, ...[]) as `0x2::bag::Bag`;
      },
      typeArgs: [] as [],
  ...
  static fromJSON(json: Record<string, any>): Bag {
    if (json.$typeName !== Bag.$typeName) {
      throw new Error(`not a Bag json object: expected '${Bag.$typeName}' but got '${json.$typeName}'`);
    }

    return Bag.fromJSONField(json);
  }

  static fromCoreObject(obj: SuiClientTypes.Object<{ content: true }>): Bag {
    if (!isBag(obj.type)) {
