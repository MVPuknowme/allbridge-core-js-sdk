...
export class TxContext implements StructClass {
  ...
  private constructor(typeArgs: [], fields: TxContextFields) {
    this.$fullTypeName = composeSuiType(TxContext.$typeName, ...typeArgs) as `0x2::tx_context::TxContext`;
  ...
  static reified(): TxContextReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(TxContext.$typeName, ...[]) as `0x2::tx_context::TxContext`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => TxContext.fromCoreObject(obj),
  ...
  static fromJSON(json: Record<string, any>): TxContext {
    if (json.$typeName !== TxContext.$typeName) {
      throw new Error(`not a TxContext json object: expected '${TxContext.$typeName}' but got '${json.$typeName}'`);
    }
