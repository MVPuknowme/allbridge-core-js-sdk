...
export class ObjectBag implements StructClass {
  ...
  private constructor(typeArgs: [], fields: ObjectBagFields) {
    this.$fullTypeName = composeSuiType(ObjectBag.$typeName, ...typeArgs) as `0x2::object_bag::ObjectBag`;
  ...
  static reified(): ObjectBagReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(ObjectBag.$typeName, ...[]) as `0x2::object_bag::ObjectBag`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => ObjectBag.fromCoreObject(obj),
  ...
  static fromJSON(json: Record<string, any>): ObjectBag {
    if (json.$typeName !== ObjectBag.$typeName) {
      throw new Error(`not a ObjectBag json object: expected '${ObjectBag.$typeName}' but got '${json.$typeName}'`);
    }
