...
export function isFeeCollector(type: string): boolean {
  ...
  return type === `${getTypeOrigin("wormhole", "fee_collector::FeeCollector")}::fee_collector::FeeCollector`;
}
...
export class FeeCollector implements StructClass {
  ...
  static get $typeName(): `${string}::fee_collector::FeeCollector` {
    return `${getTypeOrigin("wormhole", "fee_collector::FeeCollector")}::fee_collector::FeeCollector` as const;
  }
  ...
  static reified(): FeeCollectorReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(FeeCollector.$typeName, ...[]) as `${string}::fee_collector::FeeCollector`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => FeeCollector.fromCoreObject(obj),
