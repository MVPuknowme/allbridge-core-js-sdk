...
export class Bytes20 implements StructClass {
  ...
  private constructor(typeArgs: [], fields: Bytes20Fields) {
    this.$fullTypeName = composeSuiType(Bytes20.$typeName, ...typeArgs) as `${string}::bytes20::Bytes20`;
  ...
  static reified(): Bytes20Reified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(Bytes20.$typeName, ...[]) as `${string}::bytes20::Bytes20`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => Bytes20.fromCoreObject(obj),
  ...
  static fromJSON(json: Record<string, any>): Bytes20 {
    if (json.$typeName !== Bytes20.$typeName) {
      throw new Error(`not a Bytes20 json object: expected '${Bytes20.$typeName}' but got '${json.$typeName}'`);
    }
