...
export function isGuardianSet(type: string): boolean {
  ...
  return type === `${getTypeOrigin("wormhole", "guardian_set::GuardianSet")}::guardian_set::GuardianSet`;
}
...
export class GuardianSet implements StructClass {
  ...
  static get $typeName(): `${string}::guardian_set::GuardianSet` {
    return `${getTypeOrigin("wormhole", "guardian_set::GuardianSet")}::guardian_set::GuardianSet` as const;
  }
  ...
  private constructor(typeArgs: [], fields: GuardianSetFields) {
    this.$fullTypeName = composeSuiType(GuardianSet.$typeName, ...typeArgs) as `${string}::guardian_set::GuardianSet`;
  ...
  static reified(): GuardianSetReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(GuardianSet.$typeName, ...[]) as `${string}::guardian_set::GuardianSet`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => GuardianSet.fromCoreObject(obj),
  ...
  static fromJSON(json: Record<string, any>): GuardianSet {
    if (json.$typeName !== GuardianSet.$typeName) {
      throw new Error(`not a GuardianSet json object: expected '${GuardianSet.$typeName}' but got '${json.$typeName}'`);
    }
