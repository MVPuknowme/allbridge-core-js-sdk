...
export class VAA implements StructClass {
  ...
  private constructor(typeArgs: [], fields: VAAFields) {
    this.$fullTypeName = composeSuiType(VAA.$typeName, ...typeArgs) as `${string}::vaa::VAA`;
  ...
  static reified(): VAAReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(VAA.$typeName, ...[]) as `${string}::vaa::VAA`;
      },
      typeArgs: [] as [],
  ...
  static fromJSON(json: Record<string, any>): VAA {
    if (json.$typeName !== VAA.$typeName) {
      throw new Error(`not a VAA json object: expected '${VAA.$typeName}' but got '${json.$typeName}'`);
    }
