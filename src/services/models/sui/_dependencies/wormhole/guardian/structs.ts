...
export class Guardian implements StructClass {
  ...
  private constructor(typeArgs: [], fields: GuardianFields) {
    this.$fullTypeName = composeSuiType(Guardian.$typeName, ...typeArgs) as `${string}::guardian::Guardian`;
  ...
  static reified(): GuardianReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(Guardian.$typeName, ...[]) as `${string}::guardian::Guardian`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => Guardian.fromCoreObject(obj),
  ...
  static fromJSON(json: Record<string, any>): Guardian {
    if (json.$typeName !== Guardian.$typeName) {
      throw new Error(`not a Guardian json object: expected '${Guardian.$typeName}' but got '${json.$typeName}'`);
    }
