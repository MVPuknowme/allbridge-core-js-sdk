...
export class TypeName implements StructClass {
  ...
  private constructor(typeArgs: [], fields: TypeNameFields) {
    this.$fullTypeName = composeSuiType(TypeName.$typeName, ...typeArgs) as `0x1::type_name::TypeName`;
  ...
  static reified(): TypeNameReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(TypeName.$typeName, ...[]) as `0x1::type_name::TypeName`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => TypeName.fromCoreObject(obj),
  ...
  static fromJSON(json: Record<string, any>): TypeName {
    if (json.$typeName !== TypeName.$typeName) {
      throw new Error(`not a TypeName json object: expected '${TypeName.$typeName}' but got '${json.$typeName}'`);
    }
