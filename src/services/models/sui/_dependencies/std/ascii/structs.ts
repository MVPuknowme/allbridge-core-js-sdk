...
export class String implements StructClass {
  ...
  static reified(): StringReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(String.$typeName, ...[]) as `0x1::ascii::String`;
      },
      typeArgs: [] as [],
  ...
  static fromJSON(json: Record<string, any>): String {
    if (json.$typeName !== String.$typeName) {
      throw new Error(`not a String json object: expected '${String.$typeName}' but got '${json.$typeName}'`);
    }
...
export class Char implements StructClass {
  ...
  static reified(): CharReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(Char.$typeName, ...[]) as `0x1::ascii::Char`;
      },
      typeArgs: [] as [],
  ...
  static fromJSON(json: Record<string, any>): Char {
    if (json.$typeName !== Char.$typeName) {
      throw new Error(`not a Char json object: expected '${Char.$typeName}' but got '${json.$typeName}'`);
    }
