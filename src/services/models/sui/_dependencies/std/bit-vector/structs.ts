...
export class BitVector implements StructClass {
  ...
  private constructor(typeArgs: [], fields: BitVectorFields) {
    this.$fullTypeName = composeSuiType(BitVector.$typeName, ...typeArgs) as `0x1::bit_vector::BitVector`;
  ...
  static reified(): BitVectorReified {
    ...
    return {
      ...
      get fullTypeName() {
        return composeSuiType(BitVector.$typeName, ...[]) as `0x1::bit_vector::BitVector`;
      },
      typeArgs: [] as [],
      ...
      fromCoreObject: (obj: SuiClientTypes.Object<{ content: true }>) => BitVector.fromCoreObject(obj),
  ...
  static fromJSON(json: Record<string, any>): BitVector {
    if (json.$typeName !== BitVector.$typeName) {
      throw new Error(`not a BitVector json object: expected '${BitVector.$typeName}' but got '${json.$typeName}'`);
    }
